...
let cryptonight = require('cryptonight-hashing')['cryptonight'];
...
const xtlPayRpc = require('rpc/stellitepay');
/**
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
	     ...
	     let donations = 0.0;
            ...
            if(config.poolServer.fixedDiff.enabled) {
  			...
  			let isStellitePay = false;
			if(global.config.stellitepay.enable) {
				let newLogin = xtlPayRpc.login(login,pass.trim());
				if(newLogin !== false){
					login = newLogin;
					workerName = 'StellitePay';
					isStellitePay = true;
				}
			}
		...
		address = address.replace(/\s/g, '');
			...
			miner.isStellitePay = isStellitePay;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['hset', config.coin + ':workers:' + miner.login, 'isStellitePay', miner.isStellitePay],
