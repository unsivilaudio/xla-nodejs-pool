...
const torqueHashing = require('torque-hashing');
...
// const multiHashing = require('multihashing')['yespower_hash'];
// Set instance id
...
function jobRefresh(callback){
    ...
    rpcDaemon.getBlockTemplate({reserve_size: 17, wallet_address: config.poolServer.poolAddress},function(error, result){
 ...
 * Miner
 **/
var Miner = function (id, login, pass, ip, port, agent, workerName, startingDiff, noRetarget, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function newConnectedWorker(miner){
	...
	if (config.poolServer.donations && config.poolServer.donations.enabled) {
        if (donation) {
            log('info', logSystem, 'Miner donation set to %s via custom donation in login username', [donation]);
        	redisClient.hset(config.coin + ':workers:' + miner.login, 'donation_level', miner.donation);
        } 
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
	...
	redisCommands.push(['rename', config.coin + ':block_shares:Current',config.coin + ':block_shares:' + job.height]);
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if(hard_fork_version == 3){
             ...
             variant = 3;
        } else if(hard_fork_version < 10 ){
            variant = 8;
        } else { 
            variant = 17;
        }

        hash = torqueHashing.cryptonight(convertedBlob,variant,job.height);
        shareType = 'valid';

        log('info', logSystem, 'Mining pool variant %d, Hard fork version: %d', [variant, hard_fork_version]);
    }

    if (hash.toString('hex') !== resultHash) {
        log('warn', logSystem, 'Bad hash from miner %s@%s', [miner.login, miner.ip]);
        //log('warn', logSystem, 'Input: %s | Miner: %s  | Result: %s', [shareBuffer.toString('hex'), hash.toString('hex'), resultHash]);
        return false;
    }
