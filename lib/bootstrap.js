...
module.exports = function(configFile){
	...
	global.versions = [APP_VER_MAJOR,APP_VER_MINOR];
	...
	if(config.daemon.servers && config.daemon.type){
		switch(global.config.daemon.type){
			case "roundRobin" :
				for(let i in config.daemon.servers){
					const server = config.daemon.servers[i];
					if(!server.weight || server.weight === 1){
						servers.push(server);
					}else if(server.weight > 1){
						for(let w = 0;w< server.weight;w++){
							servers.push(server);	
						}
					}
				}
